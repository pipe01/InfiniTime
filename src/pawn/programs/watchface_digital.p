...
@start() {
    ...
    notification_icon = lv_label_create()
    lv_obj_set_style_local_color(notification_icon, LV_STYLE_TEXT_COLOR, 0x00FF00)
    lv_obj_align(notification_icon, lv_scr_act(), LV_ALIGN_IN_TOP_LEFT);

    heartbeat_icon = lv_label_create(.text = symbol_heartBeat)
    lv_obj_set_style_local_color(heartbeat_icon, LV_STYLE_TEXT_COLOR, 0x1B1B1B)
    lv_obj_align(heartbeat_icon, lv_scr_act(), LV_ALIGN_IN_BOTTOM_LEFT)

    heartbeat_value = lv_label_create()
    ...
    step_value = lv_label_create(.text = "0")
    lv_obj_set_style_local_color(step_value, LV_STYLE_TEXT_COLOR, 0x00FFE7)
    lv_obj_align(step_value, lv_scr_act(), LV_ALIGN_IN_BOTTOM_RIGHT)

    step_icon = lv_label_create(.text = symbol_shoe)
    ...
    label_date = lv_label_create()
    lv_obj_align(label_date, lv_scr_act(), LV_ALIGN_CENTER, 0, 60)
    lv_obj_set_style_local_color(label_date, LV_STYLE_TEXT_COLOR, 0x999999)

    label_time = lv_label_create()
    lv_obj_set_style_local_ptr(label_time, LV_STYLE_TEXT_FONT, jetbrains_mono_extrabold_compressed)
    lv_obj_align(label_time, lv_scr_act(), LV_ALIGN_IN_RIGHT_MID)

    label_time_ampm = lv_label_create()
    lv_obj_align(label_time_ampm, lv_scr_act(), LV_ALIGN_IN_RIGHT_MID, 0, -55)
}
